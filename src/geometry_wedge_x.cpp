...
void geometry::wedge_x_angle(lexer *p, dive *a, int rank, int &ts, int &te,
                double xs, double xe, double ys, double ye, double zs, double alpha)
{
	ts=p->tricount;

    ze = zs + (xe-xs)*tan(fmod(alpha,360)/180*M_PI);
    if(alpha<0)
    {
        geometry::zs = -ze;
        ze = zs;
